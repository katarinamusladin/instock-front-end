  ...
  useEffect(() => {
    const fetchWarehousesAndCategories = async () => {
      try {
        ...
        const warehouseResponse = await axios.get(`${BASE_URL}:${PORT}/api/warehouses`);
        setWarehouses(warehouseResponse.data); 
       
        const inventoryResponse = await axios.get(`${BASE_URL}:${PORT}/api/inventories`);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
        const response = await axios.post(
          `${BASE_URL}:${PORT}/api/inventories`, newItem
        );
